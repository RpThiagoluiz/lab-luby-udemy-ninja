...
*/
  ...
  const justNumbersRegex = RegExp("^\\d+", "gm");
...
*/
  ...
  const numbersAtTheEnd = RegExp("\\d+$", "gm");
...
*/
  ...
  const hasClass = (markup, cssClass) => {
    const regexCheckClass = new RegExp(`class=["'].*${cssClass}.*["']`, "gm"); //*(?:[\w\s]+)
    return regexCheckClass.test(markup);
  };

  const classes = ["container", "text", "date", "excerpt", "main"];
