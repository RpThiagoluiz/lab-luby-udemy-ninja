...
*/
  ...
  const regexHandleText = /ju\w{1}ho/g;

  console.log(text.match(regexHandleText));
...
*/
  ...
  let stringHtml =
    "<div><section><blockquote>Texto <img /></blockquote></section></div>";
  const regexHandleStringHtml = /<\w+>/g;
  console.log(stringHtml.match(regexHandleStringHtml));
...
*/
  ...
  let htmlStringTags =
    "<div><ul><li></li><li></li><li><span></span></li></ul></div>";
  const regexHandleHtmlStringTags = /<\w+><\/\w+>/g;
  console.log(htmlStringTags.match(regexHandleHtmlStringTags));
...
*/
  ...
  let stringHtml2 =
    "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>";
  const regexHandleHtml2 = /(<\w+>)([^<]*)(<\/\w+>)/g;
  const handleHtmlAndFormat = (html) =>
    html.replace(
      regexHandleHtml2,
      (arg, group1, group2, group3) =>
        `${group1}O texto dentro da tag ${group1
          .replace("<", '"')
          .replace(">", '"')} é "${group2}"${group3}\n`
    );

  console.log(handleHtmlAndFormat(stringHtml2)); //\n quebra a linha
